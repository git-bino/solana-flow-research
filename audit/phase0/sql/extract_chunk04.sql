-- Phase 0 extract — dev chunk 04.  Executed query, verbatim.
...
--
--   launch window  [2026-06-06 00:00, 2026-06-15 00:00)   9 days
--   event window   [2026-06-06 00:00, 2026-08-15 23:59)  71 days
...
-- launch filter ends 2026-06-15, 27 days before the holdout opens.  The `guard`
...
WITH sel AS (
    ...
    WHERE evt_block_date >= DATE '2026-06-06'
      AND evt_block_date <  DATE '2026-06-15'
...
guard AS (
    SELECT if(max(created_at) >= TIMESTAMP '2026-06-15 00:00:00'
              OR min(created_at) < TIMESTAMP '2026-06-06 00:00:00',
              CAST(fail('LAUNCH WINDOW GUARD: a token outside [2026-06-06, 2026-06-15)'
...
ev AS (
    ...
    WHERE t.evt_block_date >= DATE '2026-06-06'
