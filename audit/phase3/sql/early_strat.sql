...
strat AS (
    SELECT mint, max_x, y60, y115, late60,
           f_n_holders, f_gini, f_top3, f_xgrowth,
           ntile(5) OVER (ORDER BY f_xgrowth)   AS sx,
           ntile(5) OVER (ORDER BY f_n_holders) AS sh
    FROM feat WHERE NOT late60
),
-- (A) x_growth-оор стратум, дотор нь тархалтын шинжийн AUC
la AS (
    SELECT mint, y60, sx AS stratum, 'x_growth' AS strat_by, f.name, f.val
    FROM strat CROSS JOIN UNNEST(ARRAY[
        ROW('n_holders', f_n_holders), ROW('gini', f_gini), ROW('top3', f_top3)
    ]) AS f(name, val)
),
-- (B) n_holders-ээр стратум, дотор нь x_growth-ийн AUC
lb AS (
    SELECT mint, y60, sh AS stratum, 'n_holders' AS strat_by, f.name, f.val
    FROM strat CROSS JOIN UNNEST(ARRAY[
        ROW('x_growth', f_xgrowth)
    ]) AS f(name, val)
),
allx AS (SELECT * FROM la UNION ALL SELECT * FROM lb),
mr AS (
    SELECT strat_by, stratum, name, y60,
           CAST(rank() OVER (PARTITION BY strat_by, stratum, name ORDER BY val) AS double)
             + (CAST(count(*) OVER (PARTITION BY strat_by, stratum, name, val) AS double) - 1)/2.0 AS mrk
    FROM allx
)
SELECT strat_by, stratum, name,
       count_if(y60) AS n_pos, count_if(NOT y60) AS n_neg,
       CAST(count_if(y60) AS double)/count(*) AS base,
       (sum(mrk) FILTER (WHERE y60) - CAST(count_if(y60) AS double)*(count_if(y60)+1)/2.0)
         / (CAST(count_if(y60) AS double)*count_if(NOT y60)) AS auc
FROM mr GROUP BY strat_by, stratum, name
ORDER BY strat_by, name, stratum
