-- Phase 0 extract — dev chunk 05.  Executed query, verbatim.
...
--
--   launch window  [2026-06-15 00:00, 2026-06-24 00:00)   9 days
--   event window   [2026-06-15 00:00, 2026-08-15 23:59)  62 days
...
-- launch filter ends 2026-06-24, 18 days before the holdout opens.  The `guard`
...
WITH sel AS (
    ...
    WHERE evt_block_date >= DATE '2026-06-15'
      AND evt_block_date <  DATE '2026-06-24'
...
guard AS (
    SELECT if(max(created_at) >= TIMESTAMP '2026-06-24 00:00:00'
              OR min(created_at) < TIMESTAMP '2026-06-15 00:00:00',
              CAST(fail('LAUNCH WINDOW GUARD: a token outside [2026-06-15, 2026-06-24)'
...
ev AS (
    ...
    WHERE t.evt_block_date >= DATE '2026-06-15'
